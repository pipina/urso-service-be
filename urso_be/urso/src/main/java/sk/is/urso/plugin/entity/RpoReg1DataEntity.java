...
@Entity(name="RPO_1_DATA")
public class RpoReg1DataEntity extends AbstractRegEntityData {
    @ToString.Exclude
    @OneToMany(targetEntity = RpoReg1IndexEntity.class, mappedBy = "data", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    private List<AbstractRegEntityIndex> entityIndexes = new ArrayList<>();

    @ToString.Exclude
    @OneToMany(targetEntity = RpoReg1DataHistoryEntity.class, mappedBy = "zaznamId", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    private List<AbstractRegEntityDataHistory> entityDataHistory = new ArrayList<>();

    @ToString.Exclude
    @OneToMany(targetEntity = RpoReg1DataReferenceEntity.class, mappedBy = "zaznamId", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
